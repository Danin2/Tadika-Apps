...
const quests: Quest[] = [
  ...
  // Fruits
  {
    id: 'apel', letter: 'Apel', smallLetter: '🍎', title: 'APEL', subtitle: 'Apel rasanya manis!', bgColor: '#FFEBEE',
  ...
  {
    id: 'pisang', letter: 'Pisang', smallLetter: '🍌', title: 'PISANG', subtitle: 'Monyet suka pisang!', bgColor: '#FFFDE7',
  ...
  {
    id: 'jeruk', letter: 'Jeruk', smallLetter: '🍊', title: 'JERUK', subtitle: 'Jeruk banyak vitamin C!', bgColor: '#FFF3E0',
  ...
  {
    id: 'semangka', letter: 'Semangka', smallLetter: '🍉', title: 'SEMANGKA', subtitle: 'Isinya air yang segar!', bgColor: '#E8F5E9',
  ...
  {
    id: 'mangga', letter: 'Mangga', smallLetter: '🥭', title: 'MANGGA', subtitle: 'Wangi dan enak sekali!', bgColor: '#FFF9C4',
  ...
  // Animals
  {
    id: 'kucing', letter: 'Kucing', smallLetter: '🐱', title: 'KUCING', subtitle: 'Meong... meong...', bgColor: '#F5F5F5',
  ...
  {
    id: 'singa', letter: 'Singa', smallLetter: '🦁', title: 'SINGA', subtitle: 'Aummm... Raja Hutan!', bgColor: '#FFF3E0',
  ...
  {
    id: 'gajah', letter: 'Gajah', smallLetter: '🐘', title: 'GAJAH', subtitle: 'Punya belalai panjang!', bgColor: '#E1F5FE',
  ...
  {
    id: 'jerapah', letter: 'Jerapah', smallLetter: '🦒', title: 'JERAPAH', subtitle: 'Lehernya panjang sekali!', bgColor: '#FFFDE7',
  ...
  {
    id: 'monyet', letter: 'Monyet', smallLetter: '🐒', title: 'MONYET', subtitle: 'Suka panjat pohon!', bgColor: '#EFEBE9',
...
const Learning: React.FC = () => {
  ...
  // ============ AUDIO: Huruf/Angka/Buah/Hewan utama ============
  const playMainSound = useCallback(async () => {
    ...
    try {
      ...
      } else if (quest.category === 'numbers') {
        await playAudioFile(`/assets/audio/angka_${quest.letter}.mp3`);
      } else {
        // Buah & Hewan: langsung sebut namanya
        await playAudioFile(`/assets/audio/${quest.category}_${quest.id}.mp3`);
      }
    } catch {
  ...
  const playWord = useCallback(async (word: { text: string; audio: string }) => {
    ...
    try {
      if (quest.category === 'fruits' || quest.category === 'animals') {
        // Buah & Hewan: langsung sebut nama benda
        await playAudioFile(`/assets/audio/${quest.category}_${word.audio}.mp3`);
      } else {
        await playAudioFile(`/assets/audio/kata_${word.audio}.mp3`);
      }
    } catch {
      if (quest.category === 'fruits' || quest.category === 'animals') {
        await speakText(word.text.charAt(0).toUpperCase() + word.text.slice(1).toLowerCase());
      } else {
          ...
          <AnimatePresence mode="wait">
            {viewMode === 'discovery' ? (
              ...
              >
                ...
                {/* Finish Section for Fruits/Animals */}
                {(category === 'fruits' || category === 'animals') && (
                  <motion.div
                  ...
                  >
                    {/* Celebration Stars */}
                    <div className="finish-stars-row">
                      {['⭐', '🌟', '✨', '🌟', '⭐'].map((star, i) => (
                        <motion.span
                          key={i}
                    ...
                    {/* Praise Card */}
                    <div className="finish-praise-card">
                      <div className="finish-praise-emoji">
                        {category === 'fruits' ? '🍓' : '🐾'}
                      </div>
                      <div className="finish-praise-text">
                        <h3>Kamu Hebat!</h3>
                        <p>
                          Kamu sudah mengenal <strong>{quest.letter}</strong> hari ini.
                    ...
                    {/* Progress Info */}
                    <div className="finish-progress-info">
                      <span className="finish-progress-label">
                        {currentIdx + 1} dari {filteredQuests.length} selesai
                      </span>
                      <div className="finish-progress-track">
                        <div
                          className="finish-progress-fill"
                          style={{ width: `${((currentIdx + 1) / filteredQuests.length) * 100}%` }}
                        />
                      </div>
                    </div>
                    ...
                    >
                      <span className="finish-btn-emoji">🎉</span>
                      <span className="finish-btn-text">
                        {currentIdx < filteredQuests.length - 1 ? 'Lanjut Belajar!' : 'Selesai Semua!'}
                      </span>
